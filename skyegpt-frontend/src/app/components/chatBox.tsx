...
const ChatBox: React.FC<ChatBoxProps> = ({ messages, setMessages, className, conversationId }) => {
  ...
  const [animatedLoadingElements, setAnimatedLoadingElements] = useState<Array<{ id: string, text: string, animClass: string }>>([]);
  const nextAnimIdRef = useRef(0);
  const activeTimersRef = useRef<{ fadeInTimer: NodeJS.Timeout | null, cleanupTimer: NodeJS.Timeout | null }>({ fadeInTimer: null, cleanupTimer: null });
  ...
  useEffect(() => {
    const textarea = textareaRef.current;
    if (textarea) {
      textarea.focus(); textarea.style.height = INITIAL_TEXTAREA_CONTENT_HEIGHT_PX_STR;
      textareaResize(); textarea.addEventListener('input', textareaResize);
    }
    return () => {
  ...
  useEffect(() => {
    if (!isLoading) { setCurrentTextIndex(0); return; }
...
useEffect(() => {
    ...
    let nextElements: Array<{ id: string, text: string, animClass: string }> = [];
    let targetElementId = '';
  ...
  useEffect(() => {
    if (!isLoading && textareaRef.current && !isFeedbackModalOpen && !isConfirmationModalOpen) {
      const focusTimeoutId = setTimeout(() => {
        if (textareaRef.current && document.activeElement !== textareaRef.current) textareaRef.current.focus();
      }, 50);
  ...
  const debouncedHandleRating = useMemo(() => debounce(async (messageIndex: number, rating: 'thumbs-up' | 'thumbs-down') => {
    ...
    if (!conversationId) {
      ...
      setFeedbackState((prev) => ({ ...prev, [messageIndex]: previousLocalRatingState })); return;
    }
    if (newLocalRating === null) return;
    const vote: FeedbackVotePayload = newLocalRating === 'thumbs-up' ? 'positive' : 'negative';
    try { await submitFeedbackAPI(conversationId, { vote: vote, comment: "" }); }
    catch (error) {
      setFeedbackState((prev) => ({ ...prev, [messageIndex]: previousLocalRatingState }));
      const errorMessage = error instanceof Error && error.message.includes('Failed to connect') ? 'Failed to connect to the server.' : (error instanceof Error ? error.message : 'Error submitting rating.');
  ...
  const handleFeedbackPromptClick = useCallback((messageIndex: number) => {
    setActiveMessageIndex(messageIndex); setFeedbackText(''); setFeedbackError(''); setSubmitError(''); setIsFeedbackModalOpen(true);
  }, []);
  const handleFeedbackModalClose = useCallback(() => { setIsFeedbackModalOpen(false); setFeedbackError(''); setSubmitError(''); }, []);
  const handleFeedbackSubmit = useCallback(async () => {
    if (activeMessageIndex === null) { setSubmitError('No message selected for feedback.'); return; }
    if (!feedbackText.trim()) { setFeedbackError('Feedback cannot be empty.'); return; }
    setFeedbackError(''); setSubmitError('');
    if (!conversationId) { setSubmitError('conversation ID missing. Cannot submit feedback.'); return; }
    let vote: FeedbackVotePayload = 'not_specified';
    const currentRating = feedbackState[activeMessageIndex];
    if (currentRating === 'thumbs-up') vote = 'positive'; else if (currentRating === 'thumbs-down') vote = 'negative';
    try {
      await submitFeedbackAPI(conversationId, { vote: vote, comment: feedbackText.trim() });
      setIsFeedbackModalOpen(false); setIsConfirmationModalOpen(true);
    } catch (error) {
      const errorMessage = error instanceof Error && error.message.includes('Failed to connect') ? 'Failed to connect to the server.' : (error instanceof Error ? error.message : 'Error submitting feedback.');
      setSubmitError(errorMessage);
    }
  }, [activeMessageIndex, feedbackText, feedbackState, conversationId]);
  const handleConfirmationModalClose = useCallback(() => { setIsConfirmationModalOpen(false); setActiveMessageIndex(null); setFeedbackText(''); }, []);
    ...
    <div className={`flex flex-col h-full ${className || ''}`}>
      ...
      >
        {messages.map((msg, index) => (
          <MemoizedMessage
            key={index} msg={msg} index={index} isLoading={isLoading}
            showFeedbackControls={msg.sender === 'bot' && msg.text.trim() !== "" && msg.text.trim() !== "Request cancelled." && !msg.stopped && !isLoading}
            feedbackType={feedbackState[index]} currentRatingError={ratingError[index]}
            onRate={(rating) => debouncedHandleRating(index, rating)}
            onPromptFeedback={() => handleFeedbackPromptClick(index)}
          />
        ))}
        {isLoading && messages.length > 0 &&
          (messages[messages.length - 1]?.sender === 'user' || (messages[messages.length - 1]?.sender === 'bot' && messages[messages.length - 1]?.text === '')) && (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="animated-text-container flex-1">
                    {animatedLoadingElements.map(item => (
                      <span key={item.id} className={`animated-text ${item.animClass} text-sm font-bold`}>{item.text}</span>
                    ))}
      ...
      <div className="flex items-center p-2 sm:p-3 gap-2 sm:gap-3 border-t border-gray-200 bg-white shrink-0">
        <div className="flex-1 bg-gray-100 rounded-[20px] transition-all duration-200 shadow-sm flex items-end">
          <textarea
            ...
            onKeyDown={handleKeyDown} onPaste={textareaResize} onInput={textareaResize}
            placeholder="Write your question here..." disabled={isLoading} aria-label="Chat input"
          />
        </div>
        <button
          ...
          onClick={isLoading ? handleStopStreaming : sendMessage} disabled={!isLoading && !input.trim()}
          title={isLoading ? "Stop" : "Send"} aria-label={isLoading ? "Stop message" : "Send message"}
        >
          <Image src={isLoading ? "/stop.png" : "/button.png"} alt={isLoading ? "Stop" : "Send"} width={120} height={120} quality={100} style={{ width: 'auto', height: 'auto' }} priority className="object-contain"/>
        </button>
      </div>
      {isFeedbackModalOpen && activeMessageIndex !== null && (
        <div className="fixed inset-0 flex items-center justify-center bg-transparent backdrop-blur-md z-50 p-4">
          <div className="bg-white rounded-[20px] shadow-xl p-6 w-full max-w-lg flex flex-col gap-4" role="dialog" aria-labelledby="feedback-modal-header" aria-modal="true">
            <div className="flex justify-between items-center">
              <h2 id="feedback-modal-header" className="text-lg font-semibold text-gray-800 flex-1 text-center">{modalHeader}</h2>
              <button onClick={handleFeedbackModalClose} className="text-gray-500 hover:text-gray-700 text-2xl leading-none" aria-label="Close feedback modal">×</button>
            </div>
            <div className="flex flex-col gap-2 flex-1">
              <textarea
                className={`w-full min-h-[100px] p-3 rounded-[10px] bg-gray-100 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#1ea974] resize-none border ${feedbackError || submitError ? 'border-red-500 ring-red-500' : 'border-gray-300'}`}
                placeholder={textareaPlaceholder} value={feedbackText}
                onChange={(e) => { setFeedbackText(e.target.value); if (feedbackError) setFeedbackError(''); if (submitError) setSubmitError(''); }}
                aria-invalid={!!feedbackError || !!submitError} aria-describedby={feedbackError ? 'feedback-input-error' : submitError ? 'feedback-submit-error' : undefined}
              />
              {feedbackError && (<p id="feedback-input-error" className="text-red-600 text-sm">{feedbackError}</p>)}
              {submitError && (<p id="feedback-submit-error" className="text-red-600 text-sm">{submitError}</p>)}
            </div>
            <div className="flex justify-center items-center">
              <button
                onClick={handleFeedbackSubmit}
                className={`px-8 py-3 rounded-full text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${feedbackText.trim() ? 'bg-[#1ea974] hover:bg-[#17a267] focus:ring-[#1ea974]' : 'bg-gray-400 cursor-not-allowed'}`}
                disabled={!feedbackText.trim()} aria-label="Submit feedback"
              >Send</button>
            </div>
          </div>
        </div>
      ...
      {isConfirmationModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-transparent backdrop-blur-md z-50 p-4">
          <div className="bg-white rounded-[20px] shadow-xl p-6 w-full max-w-md flex flex-col gap-4 items-center" role="alertdialog" aria-labelledby="confirmation-modal-header" aria-describedby="confirmation-modal-description" aria-modal="true">
            <div className="w-full flex justify-between items-center">
              ...
              <button onClick={handleConfirmationModalClose} className="text-gray-500 hover:text-gray-700 text-2xl leading-none" aria-label="Close confirmation">×</button>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-[#17a267]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
          ...
          <div className="self-start flex flex-col w-auto max-w-[90%] sm:max-w-[80%]">
            <div className={`p-4 sm:p-5 md:p-6 rounded-[30px] transition-opacity duration-300 shadow-sm bg-[#ececec] text-black rounded-bl-[0]`}>
              <div className="prose prose-sm sm:prose-base max-w-none text-black break-words">
                <ReactMarkdown
                  ...
                  components={{
                    ol: ({ children, ...props }) => { const mergedChildren = React.Children.toArray(children).reduce<React.ReactNode[]>((acc, child) => { if (React.isValidElement(child) && child.type === 'li') acc.push(child); return acc; }, []); return <ol className="pl-6 sm:pl-8 list-decimal" {...props}>{mergedChildren}</ol>; },
                    ul: ({ node, className, children, ...props }) => (<ul className={`pl-10 sm:pl-12 list-disc ${className || ''}`} {...props}>{children}</ul>),
                    ...
                    table: ({ children }) => (<div className="overflow-x-auto w-full"><table className="min-w-full border-collapse border border-gray-300 text-xs sm:text-sm">{children}</table></div>),
                    th: ({ children }) => (<th className="border border-gray-300 bg-gray-100 px-2 sm:px-4 py-2 text-left font-semibold text-black whitespace-nowrap"><span className="block truncate">{children}</span></th>),
                    td: ({ children }) => (<td className="border border-gray-300 px-2 sm:px-4 py-2 text-black whitespace-nowrap"><span className="block truncate">{children}</span></td>),
                  }}
            ...
            {showFeedbackControls && msg.text.trim() !== '' && !msg.text.startsWith("Error:") && !msg.text.startsWith("No response received") && !msg.text.startsWith("Request cancelled") && (
              <div className="flex items-center justify-end mt-2 gap-x-1">
                <button onClick={onPromptFeedback} className="text-xs text-gray-600 hover:text-gray-900 hover:underline transition-colors duration-200" title="Provide detailed feedback" aria-label="Provide detailed feedback for this message">GIVE FEEDBACK</button>
                <div className="flex gap-x-1">
                  {(['thumbs-up', 'thumbs-down'] as const).map((ratingType) => (
                    <button
                      key={ratingType} onClick={() => onRate(ratingType)}
                      ...
                      title={ratingType === 'thumbs-up' ? "Helpful" : "Not helpful"} aria-label={ratingType === 'thumbs-up' ? "Mark as helpful" : "Mark as not helpful"} aria-pressed={feedbackType === ratingType} aria-describedby={currentRatingError ? `rating-error-${index}` : undefined}
                    ><Image src={ratingType === 'thumbs-up' ? "/tup.png" : "/tdown.png"} alt={ratingType === 'thumbs-up' ? "Thumbs Up" : "Thumbs Down"} width={16} height={16} style={{ width: 'auto', height: 'auto' }} priority quality={100} className={`object-contain ${feedbackType === ratingType && ratingType === 'thumbs-up' ? 'skgpt-tint-green-active' : ''} ${feedbackType === ratingType && ratingType === 'thumbs-down' ? 'skgpt-tint-red-active' : ''}`} /></button>
                  ))}
                </div>
              </div>
            )}
            {showFeedbackControls && currentRatingError && (<div className="flex justify-end mt-1 w-full"><p id={`rating-error-${index}`} className="text-red-500 text-xs" aria-live="polite">{currentRatingError}</p></div>)}
          </div>
        )}
      </div>
    );
  }, (prevProps, nextProps) => (
    prevProps.msg.text === nextProps.msg.text && prevProps.msg.sender === nextProps.msg.sender && prevProps.index === nextProps.index && prevProps.isLoading === nextProps.isLoading && prevProps.showFeedbackControls === nextProps.showFeedbackControls && prevProps.feedbackType === nextProps.feedbackType && prevProps.currentRatingError === nextProps.currentRatingError
  )
);

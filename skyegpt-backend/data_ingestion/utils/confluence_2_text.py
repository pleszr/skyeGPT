...
def download_public_confluence_as_text(
        url: str,
        space_key: str,
        save_path: str
):
    ...
    producer_process = create_process(target=producer_fetch_pages_from_confluence, args=(
        url,
        space_key,
        queue
    ))
    ...
    for _ in range(number_of_threads):
        consumer_process = create_process(target=consumer_save_page_as_markdown, args=(save_path,
                                                                                       queue,
                                                                                       saved_count))
    ...
    print(f"Saving confluence pages with space-key: {space_key} finished."
          f"Elapsed seconds: {time.time()-start_time:.0f}. "
          f"Saved files: {saved_count.value}")


def producer_fetch_pages_from_confluence(
        url: str,
        space_key: str,
        queue: mp.Queue
):
...
def consumer_save_page_as_markdown(
    folder_path: str,
    queue: mp.Queue,
    saved_count: mp.Value
):

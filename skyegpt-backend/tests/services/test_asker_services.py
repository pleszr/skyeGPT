...
@patch('services.asker_services.utils.async_format_to_sse')
...
async def test_agent_response_streaming_service_stream_agent_response_happy_path(
    ...
    agent_service_instance.stream_agent_response = AsyncMock(return_value=mock_stream_response)

    expected_sse_chunks = sample_objects.mock_sse_chunks
    mock_async_format_to_sse.side_effect = sample_objects.sse_stream
    ...
    mock_async_format_to_sse.assert_called_once_with(mock_stream_response)
